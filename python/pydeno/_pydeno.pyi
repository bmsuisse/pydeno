...
class RuntimeConfig:
    ...
    def __init__(
        ...
        on_console: Callable[[str, list[Any]], Any] | None = None,
        ...
        Args:
            ...
            on_console: Optional callable invoked as ``on_console(level, args)``
                for every ``console.*`` call from guest JS. ``level`` is the
                method name ("log", "info", "warn", "error", "debug", "trace")
                and ``args`` is that call's argument list. Set it to route
                console output back to Python; combine with
                ``enable_console=True`` to *also* keep the process's own
                console output. Must be synchronous.
    ...
    @property
    def on_console(self) -> Callable[[str, list[Any]], Any] | None:
        """The configured console callback, if any."""
        ...

    @on_console.setter
    def on_console(self, callback: Callable[[str, list[Any]], Any] | None) -> None:
        """Set (or clear, with None) the console callback."""
        ...
...
class Runtime:
    ...
    def termination_handle(self) -> "TerminationHandle":
        ...
        Example:
            ...
            >>> import threading
            >>> threading.Timer(2.0, handle.terminate).start()
            >>> runtime.eval("while(true){}")  # raises within ~2s, no panic
            ```
        """
        ...
...
class TerminationHandle:
    """
    Thread-safe handle for terminating a Runtime's JS execution.

    Unlike Runtime -- an ``unsendable`` pyclass whose methods panic if called
    from any thread but the one that created it -- TerminationHandle wraps
    only the Send+Sync V8 IsolateHandle, so it may be created on one thread
    (via ``Runtime.termination_handle()``) and safely used from another, e.g.
    a watchdog thread enforcing a wall-clock timeout on a blocking
    synchronous ``eval()`` call.
    """

    def terminate(self) -> None:
        """
        Interrupt the isolate's current (or next) JS execution.

        Safe to call from any thread. The Runtime's blocked synchronous
        eval() call (on its owning thread) returns/raises shortly after.
        """
        ...
